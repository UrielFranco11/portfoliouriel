...
const navLinkItems = document.querySelectorAll('.nav-links a, .nav-links button');
...
contactTriggers.forEach((trigger) => trigger.addEventListener('click', openContactDialog));
...
const copyMessage = async () => {
  ...
  try {
    ...
    formStatus.textContent = 'No se pudo copiar automáticamente. Selecciona el texto desde tu correo.';
    return false;
  }
};
...
contactForm?.addEventListener('submit', (event) => {
  ...
  const isConfigured = recipient && !recipient.includes('TU_CORREO@EJEMPLO.COM');

  if (!isConfigured) {
    formStatus.textContent = 'Falta configurar el correo de recepción en index.html antes de publicar el portafolio.';
    return;
  }
  ...
  fetch(`https://formsubmit.co/ajax/${encodeURIComponent(recipient)}`, {
    ...
    .catch(() => {
      const subject = encodeURIComponent(`Nueva consulta de ${formData.get('name')}`);
      const body = encodeURIComponent(message);
      window.location.href = `mailto:${recipient}?subject=${subject}&body=${body}`;
      formStatus.textContent = 'No hubo conexión con el formulario. Abrimos tu correo como alternativa.';
    })
    .finally(() => {
